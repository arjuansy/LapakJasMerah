...
with open('original_ProductDetailPage.tsx', 'r', encoding='utf-8') as f:
    old_content = f.read()
...
start_idx = old_content.find('  if (showQrisCode) {')
end_idx = old_content.find('  // ── REPORT MODAL ──')

ui = old_content[start_idx:end_idx]

ui = ui.replace('onClick={() => { setShowQrisCode(false); setOrdered(true); }}', '''onClick={async () => {
                  try {
...
ui = ui.replace('onClick={() => setShowOrder(true)}',
'''onClick={() => {
                const userInfo = localStorage.getItem("userInfo");
                if (!userInfo) {
...
remaining = old_content[end_idx:]

with open('src/app/pages/ProductDetailPage.tsx', 'w', encoding='utf-8') as f:
    f.write(imports + ui + remaining)

print("Restored with python script!")
